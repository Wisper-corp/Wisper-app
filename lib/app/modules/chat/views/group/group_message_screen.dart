...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  void initState() {
    ...
    _controllerTag = 'chat_${widget.chatId ?? 'unknown'}';
    ctrl = Get.isRegistered<MessageController>(tag: _controllerTag)
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // ✅ Messages cached থাকলে সাথে সাথে bottom এ যাও
  ...
  void _handleNewMessages() {
    ...
    // ✅ Network থেকে প্রথমবার load হলে bottom এ যাও
    ...
    if (currentCount > _previousMessageCount) {
      ...
      if (isMyMessage) {
        // ✅ আমি পাঠালে সবসময় নিচে যাবে
        Future.delayed(const Duration(milliseconds: 100), () {
          if (mounted) _scrollToBottom();
        });
      } else if (_isAtBottom) {
        // অন্যজন পাঠালে এবং আমি নিচে থাকলে auto scroll
        Future.delayed(const Duration(milliseconds: 50), () {
          if (mounted) _scrollToBottom();
        });
      } else {
        // অন্যজন পাঠালে এবং আমি উপরে থাকলে indicator দেখাও
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    final targetChatId = widget.chatId;
    if (targetChatId != null &&
        targetChatId.isNotEmpty &&
        ctrl.currentChatId != targetChatId &&
        _lastEnsuredChatId != targetChatId) {
      _lastEnsuredChatId = targetChatId;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        ctrl.setupChat(chatId: targetChatId);
      });
    }
